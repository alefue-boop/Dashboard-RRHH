...
# =====================================================================
# PEGA AQUÍ TU ENLACE NORMAL DE GOOGLE SHEETS (Puede ser el del navegador)
# =====================================================================
URL_MULTAS = "bot-dashboard@dashboard-rrhh-tafca.iam.gserviceaccount.com"
URL_CONTRATOS = "bot-dashboard@dashboard-rrhh-tafca.iam.gserviceaccount.com"
...
def cargar_desde_drive(url, tipo="multas"):
    ...
    # TRADUCTOR AUTOMÁTICO DE LINKS: Convierte un link normal en link de datos
    if "/edit" in url:
        url = url.split("/edit")[0] + "/export?format=csv"
        
    try:
        # Intentamos leer la nube
        df = pd.read_csv(url, encoding="utf-8", on_bad_lines="skip")
        
        # Si Google nos devuelve un error de privacidad (HTML) en vez de los datos:
        if len(df.columns) > 0 and str(df.columns[0]).strip().startswith('<'):
            return pd.DataFrame(), "🔒 **Tu archivo de Google Drive está Privado.** Debes ir al botón Compartir y cambiarlo a 'Cualquier persona con el enlace'."
            
        df.columns = df.columns.str.strip()
        
        # Limpieza específica para Multas
        if tipo == "multas":
            ...
            if 'Costo Monetario' in df.columns and 'Año' in df.columns:
                ...
                return df, "OK"
            return pd.DataFrame(), "❌ El archivo se leyó, pero no se encontraron las columnas 'Costo Monetario' y 'Año'."
        
        # Limpieza para Contratos
        return df, "OK"
        
    except Exception as e:
        return pd.DataFrame(), f"❌ No se pudo descargar el archivo. El enlace podría estar roto."
